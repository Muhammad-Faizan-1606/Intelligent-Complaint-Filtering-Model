...
def _format_email(case: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, str]:
    ...
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif;line-height:1.5;">
      ...
      <p style="margin:0 0 6px;"><b>Decision:</b> {result.get('decision')}</p>
      <p style="margin:0 0 6px;"><b>Confidence:</b> {result.get('confidence')}</p>
      <p style="margin:0 0 12px;"><b>Source:</b> {result.get('source')}</p>
      <div style="padding:10px 12px;border:1px solid #e5e7eb;border-radius:10px;background:#f9fafb;">
        <b>Reason</b><br>{(result.get('reason') or '').replace('\n','<br>')}
      </div>
